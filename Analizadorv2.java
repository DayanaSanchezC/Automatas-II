...
public class Analizadorv2 {
                    ...
                    if (token.contains("$") || token.contains("%") || token.contains("#") || token.contains("&")) {
                        ...
                        if (partes.length > 0) {
                            ...
                            if (!tablaSimbolos.containsKey(nombre)) {
                                ...
                                bw.write(String.format("%-15s", nombre));
    ...
    private static void generarTablaDirecciones() {
        try (BufferedWriter bw = new BufferedWriter(new FileWriter("tabla_direcciones.txt"))) {
            bw.write(String.format("%-15s%-15s%-15s%-10s", "ID", "TOKEN", "VALOR", "AMBITO"));
            bw.newLine();

            for (String nombre : tablaDirecciones.keySet()) {
                String tipo = tablaSimbolos.get(nombre);
                bw.write(String.format("%-15s", nombre));
                bw.write(String.format("%-15s", tipo));
                bw.write(String.format("%-15s", "1")); // Valor predeterminado
                bw.write(String.format("%-10s", "0")); // Ámbito predeterminado
                bw.newLine();
            }
    ...
    public static void generarNuevaTabla() {
        try {
            ...
            while ((linea = br.readLine()) != null) {
                ...
                bw.write(String.format("%s|%-15s|%d|%s", nombre, partes[1], posicion, partes[3]));
                bw.newLine();
            }
